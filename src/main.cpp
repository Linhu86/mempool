...
bool memoryStandardUnitTest(MemoryPool* pool, bool dumpMemoryStates)
{
  ...
  if(*(ptr+100) != pool->s_trashOnCreation) 
    return false;

  // Check trash on alloc
  if(*ptr != pool->s_trashOnAllocSignature)
    return false;

  // Check trash on Free
  pool->free(ptr);

  if( *ptr != pool->s_trashOnFreeSignature)
    return false;
  ...
  if(hasBoundsCheckOn && pool->integrityCheck() != true)
    return false;
  ...
  if(hasBoundsCheckOn)
  {
    ...
    *block[3] = 'A';
    ...
    // This copy is fine, lets check this
    if(pool->getFreePoolSize() != pool->getTotalPoolSize() - poolSizeOffset)
      return false;
